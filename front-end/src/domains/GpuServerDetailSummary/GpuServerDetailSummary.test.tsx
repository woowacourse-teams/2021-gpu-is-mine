...
describe("GpuServerDetailSummary", () => {
  test("data", () => {
    render(<GpuServerDetailSummary detail={gpuServerResponse} serverId={1} labId={1} />);
    ...
    expect(screen.getByText(gpuServerResponse.memorySize, { exact: false })).toBeInTheDocument();

    expect(screen.getByText(/디스크 용량/)).toBeInTheDocument();
    expect(screen.getByText(gpuServerResponse.diskSize, { exact: false })).toBeInTheDocument();
    ...
    expect(
      screen.getByText(gpuServerResponse.gpuBoard.performance, { exact: false })
    ).toBeInTheDocument();
